...
### 1. ToolInvocation 接口 - 工具调用抽象

这是工具系统的**核心抽象**，定义了一个已验证且准备执行的工具调用。

```typescript
// lines 25-66: 工具调用接口设计
...
### 2. ToolBuilder 接口 - 构建器模式

实现了**构建器设计模式**，分离工具定义和实例化逻辑。

```typescript
// lines 246-291: 工具构建器接口
...
### 2. DeclarativeTool - 声明式工具基类

实现了**声明式工具设计模式**，分离验证和执行逻辑。

```typescript
// lines 297-418: 声明式工具基类
...
### 3. BaseDeclarativeTool - 默认验证实现

提供了**两层验证架构**的默认实现。

```typescript
// lines 427-467: 默认验证实现
...
classDiagram
    ...
    class DeclarativeTool {
        ...
        +buildAndExecute(): Promise
        +validateBuildAndExecute(): Promise
        +validateToolParams(): string | null
        #abstract build(): ToolInvocation
    }
    ...
    class BaseDeclarativeTool {
        ...
        +validateToolParams(): string | null
        +validateToolParamValues(): string | null
        #abstract createInvocation(): ToolInvocation
    }

    class ConcreteToolImpl {
        +createInvocation(): ToolInvocation
    }

    class ConcreteInvocationImpl {
        +execute(): Promise~TResult~
        +getDescription(): string
    }

    ToolInvocation <|-- BaseToolInvocation
    BaseToolInvocation <|-- ConcreteInvocationImpl
    ToolBuilder <|-- DeclarativeTool
    DeclarativeTool <|-- BaseDeclarativeTool
    BaseDeclarativeTool <|-- ConcreteToolImpl
    ConcreteToolImpl --> ConcreteInvocationImpl : creates
```
...
```typescript
// lines 519-597: JSON Schema 循环检测
...
_分析日期: 2024年11月14日_ _文件路径: packages/core/src/tools/tools.ts_
