...
我一直有这么一个疑问： 为什么那么多的框架表现出来的效果差距这么大？即便同一个模型效果也是相差甚远？一直带着这个问题分析各个框架，包括 openmanus，unas,
oxygent, deepcode, kode, gemini
cli，包括自己写的一个java版的这类的agent框架，以及公司内部的joycode， 这些框架， 在不断的尝试中找出产生差距的原因，总结下来：从大体上一看思路都差不多，但是表现出来的效果确天差地别。why？

1. 模型不一致？ 这个我们可以自己替换看效果，有大部分效果变化不是很大， 除非模型的能力差距很大的除外
2. 提示词（尤其系统提示词）？ 这个我分析的比较少，但是这个影响非常厉害，在自己开发agent框架里面体现的非常明显。
3. 上下文处理不一样导致的，这个比较难对比后面想办法对比一下？
4. memory和上下文有点类似但是也有很大的区别，记忆需要准，删除无用历史，持久化这些对模型的产出有没有影响？
5. 工具选择？还是内置工具效果不一样需要找到真正的原因，也许是多重原因重叠起来导致的，找到调试的最佳姿势带着这么目的，去分析也许能做出来一个类似claudde
   code那么强的工具或者比他更强

去深入分析一下这类的开源代码， 不断是去尝试改造这些变量
